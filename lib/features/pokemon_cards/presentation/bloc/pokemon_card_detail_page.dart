      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            if ((card.weaknesses?.isNotEmpty ?? false) || (card.resistances?.isNotEmpty ?? false)) ...[
              const SizedBox(height: 24),
  ...
  Widget _buildCardInfoSection(TextTheme textTheme) {
    return _buildSection(
      ...
      children: [
        ...
        if (card.subtypes?.isNotEmpty ?? false)
        ...
        if (card.weaknesses?.isNotEmpty ?? false)
          Expanded(
            child: _buildSection(
              title: 'Debilidades',
              textTheme: textTheme,
              children: card.weaknesses!.map((w) => Text('${w.type} ${w.value}', style: textTheme.bodyLarge)).toList(),
            ),
          ),
        if (card.resistances?.isNotEmpty ?? false)
          Expanded(
            child: _buildSection(
              title: 'Resistencias',
              textTheme: textTheme,
              children: card.resistances!.map((r) => Text('${r.type} ${r.value}', style: textTheme.bodyLarge)).toList(),
            ),
          ),
      ],
    );
